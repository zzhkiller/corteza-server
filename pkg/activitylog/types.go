...
type (
	...
	// Activity Standardized data structure for resource activity
	Activity struct {
	...
	Filter struct {
		...
		Check func(*Activity) (bool, error) `json:"-"`
	...
	// @fixme too many interface; too many Type assertions in CastToActivity
...
func CastToActivity(dec interface{}) (a *Activity, err error) {
	a = &Activity{
		ID:             id.Next(),
		Timestamp:      time.Now(),
